...
class Tennis3DSystem:
    ...
    def predict(self, kpts_2d_sequence):
        ...
        kpts_combined = np.concatenate([kpts_3d_coco, conf_channel], axis=2)

        # --- D. NORMALIZE ---
        # 1. Normalize (Center + Rotate)
        kpts_norm = normalize_skeleton(kpts_combined) 
        

        # --- E. PREPARE TENSOR ---
        data_joint = torch.tensor(kpts_norm, dtype=torch.float32).to(self.device)
        data_joint = data_joint.unsqueeze(0).unsqueeze(-1) # (1, 4, 40, 17, 1)
